...
type LogEntry = {
  ...
  entity: "worker" | "equip" | "project" | "timesheet" | "company";
  entityId: string; action: string; details?: string;
};
...
type TimeEntry = {
  ...
  workerId: string;
  projectId: string;
  companyId: string;     // vendor partition
...
export default function Board(){
  ...
  // Time sample (simple table; future: supervisor app)
  ...
  function logEvent(e: Omit<LogEntry,"id"|"ts"|"actor">){
    setLogs(prev => [{ id: crypto.randomUUID(), ts: new Date().toISOString(), actor:"Sam", ...e }, ...prev]);
  }
  ...
  /* ---------- Drag helpers ---------- */
  function onDragStart(ev: React.DragEvent, payload: any){
  ...
  function dropWorker(pid: string, ev: React.DragEvent){
    ...
    try{
      ...
      setCrewByProject(prev=>{
        ...
        // remove from previous, preserve tag
  ...
  function cycleTag(pid: string, idx: number){
    setCrewByProject(prev=>{
      ...
      const next = list.map((s,i)=>{
        if(i!==idx) return s;
        const tag: RoleTag = s.roleTag==="Lead" ? "Assistant" : s.roleTag==="Assistant" ? null : "Lead";
        return { ...s, roleTag: tag };
      });
  ...
  function generateReport(){
    // simple report preview, left in as demo
    const lines: string[] = [];
    projects.forEach(p=>{
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 text-slate-100">
      ...
      <div className="ml-56 p-6">
          ...
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-30">
            <div className="w-full max-w-lg rounded-xl bg-slate-900 border border-white/10 p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <label className="col-span-2">Name<input className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={newWorker.name||""} onChange={e=>setNewWorker(w=>({...w, name:e.target.value}))}/></label>
                <label>Role<input className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={newWorker.role||"Worker"} onChange={e=>setNewWorker(w=>({...w, role:e.target.value}))}/></label>
                <label>Certs (comma)<input className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={Array.isArray(newWorker.certs)?newWorker.certs.join(", "):(newWorker.certs as any)||""} onChange={e=>setNewWorker(w=>({...w, certs:e.target.value}))}/></label>
                <label className="col-span-2">Company
                  <select className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={newWorker.companyId} onChange={e=>setNewWorker(w=>({...w, companyId:e.target.value}))}>
                    {companies.map(c=> <option key={c.id} value={c.id}>{c.name}</option>)}
                  </select>
                </label>
              </div>
              <div className="flex justify-end gap-2">
                <button onClick={()=>{setShowAddWorker(false)}} className="px-3 py-2 rounded bg-white/5 border border-white/10">Cancel</button>
                <button onClick={addWorker} className="px-3 py-2 rounded bg-emerald-500/20 border border-emerald-400/30">Add</button>
              </div>
            </div>
          </div>
        )}
        ...
        {showAddEquip && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-30">
            <div className="w-full max-w-lg rounded-xl bg-slate-900 border border-white/10 p-4 space-y-3">
              <div className="text-lg font-semibold">Add Equipment</div>
              <div className="grid grid-cols-2 gap-3 text-sm">
                <label className="col-span-2">Code<input className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={newEquip.code||""} onChange={e=>setNewEquip(s=>({...s, code:e.target.value}))}/></label>
                <label>Kind<input className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={newEquip.kind||""} onChange={e=>setNewEquip(s=>({...s, kind:e.target.value}))}/></label>
                <label>Status
                  <select className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={newEquip.status||"active"} onChange={e=>setNewEquip(s=>({...s, status:e.target.value as any}))}>
                    <option>active</option><option>idle</option><option>repair</option>
                  </select>
                </label>
              </div>
              <div className="flex justify-end gap-2">
                <button onClick={()=>setShowAddEquip(false)} className="px-3 py-2 rounded bg-white/5 border border-white/10">Cancel</button>
                <button onClick={addEquip} className="px-3 py-2 rounded bg-emerald-500/20 border border-emerald-400/30">Add</button>
              </div>
            </div>
          </div>
        )}
        ...
        {showAddProject && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-30">
            <div className="w-full max-w-lg rounded-xl bg-slate-900 border border-white/10 p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <label className="col-span-2">Name<input className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={newProject.name||""} onChange={e=>setNewProject(s=>({...s, name:e.target.value}))}/></label>
                <label>Target Crew<input type="number" min={0} className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={Number(newProject.targetCrew||0)} onChange={e=>setNewProject(s=>({...s, targetCrew:Number(e.target.value||0)}))}/></label>
                <label>Target Equip<input type="number" min={0} className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={Number(newProject.targetEquip||0)} onChange={e=>setNewProject(s=>({...s, targetEquip:Number(e.target.value||0)}))}/></label>
              </div>
              <div className="flex justify-end gap-2">
                <button onClick={()=>setShowAddProject(false)} className="px-3 py-2 rounded bg-white/5 border border-white/10">Cancel</button>
                <button onClick={addProject} className="px-3 py-2 rounded bg-emerald-500/20 border border-emerald-400/30">Add</button>
              </div>
            </div>
          </div>
        )}
        ...
        {showAddCompany && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-30">
            <div className="w-full max-w-lg rounded-xl bg-slate-900 border border-white/10 p-4 space-y-3">
              <div className="text-lg font-semibold">Add Company</div>
              <div className="grid grid-cols-2 gap-3 text-sm">
                <label className="col-span-2">Name<input className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={newCompany.name||""} onChange={e=>setNewCompany(s=>({...s, name:e.target.value}))}/></label>
                <label>Contact<input className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={newCompany.contactName||""} onChange={e=>setNewCompany(s=>({...s, contactName:e.target.value}))}/></label>
                <label>Phone<input className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={newCompany.phone||""} onChange={e=>setNewCompany(s=>({...s, phone:e.target.value}))}/></label>
                <label className="col-span-2">Email<input className="w-full mt-1 bg-slate-800 border border-slate-700 rounded px-2 py-1" value={newCompany.email||""} onChange={e=>setNewCompany(s=>({...s, email:e.target.value}))}/></label>
              </div>
              <div className="flex justify-end gap-2">
                <button onClick={()=>setShowAddCompany(false)} className="px-3 py-2 rounded bg-white/5 border border-white/10">Cancel</button>
                <button onClick={addCompany} className="px-3 py-2 rounded bg-emerald-500/20 border border-emerald-400/30">Add</button>
              </div>
            </div>
          </div>
        )}
